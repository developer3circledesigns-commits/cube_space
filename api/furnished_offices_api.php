...
if ($action === 'list') {
    ...
    $cityRes = mysqli_query($conn, "SELECT city, COUNT(*) as cnt FROM furnished_offices WHERE status='published' GROUP BY city ORDER BY cnt DESC");
    ...
    $areaStmt = mysqli_prepare($conn,
        "SELECT area, COUNT(*) as cnt FROM furnished_offices WHERE status='published' $areaFilterSql AND area IS NOT NULL GROUP BY area ORDER BY cnt DESC"
    );
    if (!empty($areaParams)) {

...
if ($action === 'autocomplete') {
    ...
    $cacheKey = 'ac_furnished_v3_' . JsonCache::getGlobalVersion() . '_' . md5($query);
...
if ($action === 'list') {
    ...
    $countSql = "SELECT COUNT(*) as total FROM furnished_offices WHERE $whereClause";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $row['images_arr'] = resolve_images_for_row($row['images'] ?? '[]', 'furnished', $row['id'] ?? 0, $conn);
    ...
    $cityRes = mysqli_query($conn, "SELECT city, COUNT(*) as cnt FROM furnished_offices WHERE status='active' GROUP BY city ORDER BY cnt DESC");
    ...
    $areaStmt = mysqli_prepare($conn,
        "SELECT area, COUNT(*) as cnt FROM furnished_offices WHERE status='active' $areaFilterSql AND area IS NOT NULL GROUP BY area ORDER BY cnt DESC"
    );
    if (!empty($areaParams)) {
    ...
    if (!empty($offices) && $total > 0) {
        ...
        if ($coordCount > 0 && !empty($excludeIds)) {
            ...
            while ($r = mysqli_fetch_assoc($nearRes)) {
                ...
                $r['images_arr'] = resolve_images_for_row($r['images'] ?? '[]', 'furnished', $r['id'] ?? 0, $conn);
...
if ($action === 'map') {
    ...
    $features = [];
    while ($row = mysqli_fetch_assoc($result)) {
        $imagesArr = resolve_images_for_row($row['images'] ?? '[]', 'furnished', $row['id'] ?? 0, $conn);

...
<body>
            ...
            <div class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body">
                    ...
                    <form id="callbackForm" onsubmit="handleCallbackForm(event)">
                        <input type="hidden" name="interest" value="furnished">
                        <input type="hidden" name="source" value="callback_furnished">
                        <div class="mb-3">
    ...
    <span style="font-weight:normal; text-transform:none;">( Negotiable )</span>
                                                ...
                                                <div class="card-actions">
                                                    <span class="btn btn-get-price" role="button" tabindex="0" data-office-id="${o.id}" data-listing-code="${escHtml(o.listing_code)}" onclick="event.stopPropagation();openGetPriceModal(${o.id}, '${escHtml(o.listing_code)}', '${escHtml(o.title)}', '${escHtml(o.listing_type_db || 'furnished')}')" onkeydown="if(event.key==='Enter'){event.stopPropagation();openGetPriceModal(${o.id}, '${escHtml(o.listing_code)}', '${escHtml(o.title)}', '${escHtml(o.listing_type_db || 'furnished')}')}">Get Best Price</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                html += MultiSelectEnquiry.renderCardShell({
                    id: o.id,
                    slug: o.slug,
                    listingType: o.listing_type_db || 'furnished',
        ...
        function doLoadListings() {
            ...
            fetch(apiUrl(apiEndpoint + '?' + qs), { cache: 'no-store', credentials: 'same-origin', headers: { 'Pragma': 'no-cache', 'Cache-Control': 'no-cache' } })
                .then(r => r.json())
                .catch(() => {});
        }
        ...
        function openGetPriceModal(officeId, listingCode, workspaceName, listingType) {
            ...
            if (interestEl) {
                var t = (listingType || 'furnished').toString().trim().toLowerCase();
        ...
        //  INIT
        // ============================================================
        // Shared storageKey for furnished+unfurnished is intentional: officeKey is type-prefixed
        // (furnished:5 vs unfurnished:5) so IDs do not collide; commercial page stores both.
        MultiSelectEnquiry.init({
            interest: 'furnished',
            storageKey: 'cubespace_multi_select_furnished'
        });
        FilterPanelToggle.init({ storageKey: 'cubespace_filters_open_furnished' });
        loadListings();
    </script>
